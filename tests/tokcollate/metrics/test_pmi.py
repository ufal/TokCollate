...
def test_pmi_aggregation_methods():
    ...
    metric_mean = build_metric(
        metric="pointwise_mutual_information",
        metric_label="test_pmi_mean",
        aggregation="mean"
    )
    assert metric_mean._aggregate_pmi(pmi_scores) == 3.0
    
    metric_sum = build_metric(
        metric="pointwise_mutual_information",
        metric_label="test_pmi_sum",
        aggregation="sum"
    )
    assert metric_sum._aggregate_pmi(pmi_scores) == 15.0
    
    metric_median = build_metric(
        metric="pointwise_mutual_information",
        metric_label="test_pmi_median",
        aggregation="median"
    )
    assert metric_median._aggregate_pmi(pmi_scores) == 3.0
...
def test_pmi_identical_sentences():
    ...
    metric = build_metric(
        metric="pointwise_mutual_information",
        metric_label="test_pmi_identical",
        aggregation="mean"
    )
