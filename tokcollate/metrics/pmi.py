...
class PointwiseMutualInformationMetric(TokCollateMultilingualMetric):
    ...
    def score(self, data: TokCollateData, system_label: str, src_lang: str, tgt_lang: str) -> float:
        ...
        vocab_src = self._extract_vocabulary(data.get_system_text(system_label=system_label, language=src_lang), 
                                            self.vocab_most_common)
        vocab_tgt = self._extract_vocabulary(data.get_system_text(system_label=system_label, language=tgt_lang),
                                            self.vocab_most_common)
        ...
        for i, src_lang in enumerate(languages):
            for j, tgt_lang in enumerate(languages):
                if i == j:
                    ...
                    pmi_scores = self._compute_pmi(
                        texts[src_lang], texts[tgt_lang], 
                        vocabs[src_lang], vocabs[tgt_lang]
                    )
    ...
    def _compute_pmi(
        ...
        for src_sent, tgt_sent in zip(text_src, text_tgt):
