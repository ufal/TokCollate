...
- `tokenizations.json.gz` (optional) — compressed tokenized text data for visualization. This file is automatically generated by the TokCollate tool and contains the actual tokenizations for each tokenizer and language. When present, the visualizer can display the tokenized text alongside the metrics.
